...
the one payload Rich lays out itself -- borderless plain text when piped, the
same layout plus color on a tty. Rich honors ``NO_COLOR``, ``FORCE_COLOR`` and
tty detection automatically.
"""
...
# ANSI (16-color) themes adapt to the user's own terminal palette rather than
# hard-coding a scheme.
_SYNTAX_THEME = "ansi_dark"
_ID_STYLE = "green"  # color for the search table's id column
# Width far past any table; see emit_table for why.
...
def emit_code(text: str, language: str) -> None:
    ...
    if not _out.is_terminal:
        print(text)
        return
    # ansi_dark's own background is already the terminal default, so no
    # background_color override is needed.
    _out.print(
        Syntax(text, language, word_wrap=False, theme=_SYNTAX_THEME),
        crop=False,
    )
...
def emit_table(st: SearchTable) -> None:
    ...
    A borderless table at its natural width: color on the header row and the
    id column on a tty, plain text when piped (Rich drops styling off a
    terminal automatically). The natural width is forced so Rich never fits the
    table to the terminal (which would wrap, truncate, or drop columns); the
    terminal soft-wraps any overflow itself, so nothing is hidden.
    """
    ...
    # A fresh console on the same file auto-detects color (tty) or plain text
    # (piped). Its width is forced far past any table so Rich lays out the natural
    # width (a non-expand Table never grows to fill) rather than shrinking the
    # no_wrap columns to the terminal; the terminal soft-wraps any overflow.
    Console(file=_out.file, width=_UNBOUNDED_WIDTH).print(table, crop=False)
